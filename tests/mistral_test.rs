use asum::summarizer::{AIConfig, Summarizer, mistral::MistralProvider};
...
fn test_mistral_provider_new() {
    ...
    let provider = MistralProvider::new(ai_config);
...
#[tokio::test]
async fn test_mistral_summarize_missing_key() {
    let ai_config = AIConfig {
        model: "mistral-small-latest".to_string(),
        temperature: 0.7,
        top_p: 1.0,
    ...
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains("Mistral API key is missing")
    );
}

#[tokio::test]
async fn test_mistral_summarize_success() {
    ...
    tokio::spawn(async move {
        ...
        let response = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n{\"choices\": [{\"message\": {\"content\": \"fix: mistral success\"}}]}";
    ...
    assert_eq!(result, "fix: mistral success");
}

#[tokio::test]
async fn test_mistral_summarize_api_error() {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let url = format!("http://{}", addr);
...
#[tokio::test]
async fn test_mistral_summarize_empty_response() {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let url = format!("http://{}", addr);
...
#[tokio::test]
async fn test_mistral_summarize_rate_limit_retry() {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let url = format!("http://{}", addr);
    ...
    tokio::spawn(async move {
        ...
        let _ = tokio::io::AsyncReadExt::read(&mut socket2, &mut buf)
            .await
            .unwrap();
        let response_200 = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n{\"choices\": [{\"message\": {\"content\": \"fix: mistral success after retry\"}}]}";
    ...
    let provider = MistralProvider::new(ai_config);
    let result = provider.summarize("diff").await.unwrap();
    assert_eq!(result, "fix: mistral success after retry");
}
