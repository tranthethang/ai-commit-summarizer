//! GitHub Models AI provider for ASUM.
//!
//! This module implements the `Summarizer` trait using the GitHub Models API
...
/// Implementation of the `Summarizer` trait using GitHub Models API.
...
impl GithubProvider {
    /// Creates a new instance of `GithubProvider`.
...
#[async_trait]
impl Summarizer for GithubProvider {
    /// Generates a commit summary using the GitHub Models API.
    ...
    async fn summarize(&self, diff: &str) -> anyhow::Result<String> {
        let api_key = self
            ...
            .api_key
            .as_deref()
            .context("GitHub API key is missing")?;
        ...
        let response = loop {
            ...
            if res.status() == reqwest::StatusCode::TOO_MANY_REQUESTS && retries < max_retries {
                retries += 1;
                warn!(
                    "GitHub Models API rate limited (429). Retrying in {}s... (Attempt {}/{})",
            ...
            let res = check_response_status(res, "GitHub Models API").await?;
            break res;
        };
