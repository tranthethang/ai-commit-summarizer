//! Groq AI provider for ASUM.
//!
//! This module implements the `Summarizer` trait using Groq's API
...
/// Implementation of the `Summarizer` trait using Groq's API.
...
impl GroqProvider {
    /// Creates a new instance of `GroqProvider`.
...
#[async_trait]
impl Summarizer for GroqProvider {
    /// Generates a commit summary using the Groq API.
    ...
    async fn summarize(&self, diff: &str) -> anyhow::Result<String> {
        let api_key = self
            ...
            .api_key
            .as_deref()
            .context("Groq API key is missing")?;
        ...
        let response = loop {
            ...
            if res.status() == reqwest::StatusCode::TOO_MANY_REQUESTS && retries < max_retries {
                retries += 1;
                warn!(
                    "Groq API rate limited (429). Retrying in {}s... (Attempt {}/{})",
            ...
            let res = check_response_status(res, "Groq API").await?;
            break res;
        };
